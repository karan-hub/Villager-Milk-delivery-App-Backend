...
public interface SubscriptionEventsRepository
        extends JpaRepository<SubscriptionEvents, UUID> {
    ...
    List<SubscriptionEvents> findBySubscription_IdAndDeliveryDateAfter(
            UUID subscriptionId,
            LocalDate date
    );
    ...
    long countByDeliveryDateAndStatus(
            LocalDate date,
            EventStatus status
    );

    long countByDeliveryDateBetween(
            LocalDate startDate,
            LocalDate endDate
    );

    long countByDeliveryDateBetweenAndStatus(
            LocalDate startDate,
            LocalDate endDate,
            EventStatus status
    );
